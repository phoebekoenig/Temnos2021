...
```{r}
workerdata$standardizedvenom<-workerdata$Venom.Volume
...
venombyratio=ggplot(data=workerdata, aes(Pupae.Adult.Ratio,standardizedvenom, color=Behavior.Code)) + 
geom_point() +
geom_smooth(method=lm, fill="gray", se=TRUE) +
...
model1<-lme(standardizedvenom~Callow+Collection.Day+Behavior.Code*Adult.Count.Scaled+Behavior.Code*Pupae.Adult.Ratio+as.factor(Queen.Count>0)+Larvae.Adult.Ratio+Winged.Queen.Count, data=workerdata, random= ~1|Colony)
summary(model1)
...
model2<-lme(standardizedvenom~Callow+Collection.Day+Behavior.Code*Adult.Count.Scaled+Behavior.Code*Pupae.Adult.Ratio+as.factor(Queen.Count>0)+Winged.Queen.Count, data=workerdata, random= ~1|Colony)
summary(model2)
...
model3<-lme(standardizedvenom~Callow+Behavior.Code*Adult.Count.Scaled+Collection.Day+Behavior.Code*Pupae.Adult.Ratio+as.factor(Queen.Count>0), data=workerdata, random= ~1|Colony)
summary(model3)
...
model3_OutliersRemoved<-lme(standardizedvenom~Callow+Behavior.Code*Adult.Count.Scaled+Collection.Day+Behavior.Code*Pupae.Adult.Ratio+as.factor(Queen.Count>0), data=subset(workerdata,Pupae.Adult.Ratio<5), random= ~1|Colony)
...
Which is the best way to control for Weber's length? Do we standardize response variable or include it in the model? Do we cube it or leave it linear?
```{r}
#our selected model
model3<-lme(standardizedvenom~Callow+Behavior.Code*Adult.Count.Scaled+Collection.Day+Behavior.Code*Pupae.Adult.Ratio+as.factor(Queen.Count>0), data=workerdata, random= ~1|Colony)
summary(model3)
...
#Selected model with response variable standardized by weber's length cubed 
workerdata$standardizedvenom2<-workerdata$Venom.Volume/(workerdata$Webers.Length.mm^3)
model3_StandCubed<-lme(standardizedvenom2~Callow+Behavior.Code*Adult.Count.Scaled+Collection.Day+Behavior.Code*Pupae.Adult.Ratio+as.factor(Queen.Count>0), data=workerdata, random= ~1|Colony)
summary(model3_StandCubed)
emtrends(model3_StandCubed,pairwise~Behavior.Code, var = "Pupae.Adult.Ratio",infer=TRUE)
emtrends(model3_StandCubed,pairwise~Behavior.Code, var = "Adult.Count.Scaled",infer=TRUE)

#changed so venom volume is reponse variable and weber's length is fixed effect
model3_Volume<-lme(Venom.Volume~Callow+Webers.Length.mm+Behavior.Code*Adult.Count.Scaled+Collection.Day+Behavior.Code*Pupae.Adult.Ratio+as.factor(Queen.Count>0), data=workerdata, random= ~1|Colony)
summary(model3_Volume)
emtrends(model3_Volume,pairwise~Behavior.Code, var = "Pupae.Adult.Ratio",infer=TRUE)
emtrends(model3_Volume,pairwise~Behavior.Code, var = "Adult.Count.Scaled",infer=TRUE)

model3_VolumeCubed<-lme(standardizedvenom~Callow+Webers.Length.mm^3+Behavior.Code*Adult.Count.Scaled+Collection.Day+Behavior.Code*Pupae.Adult.Ratio+as.factor(Queen.Count>0), data=workerdata, random= ~1|Colony)
summary(model3_VolumeCubed)
emtrends(model3_VolumeCubed,pairwise~Behavior.Code, var = "Pupae.Adult.Ratio",infer=TRUE)
emtrends(model3_VolumeCubed,pairwise~Behavior.Code, var = "Adult.Count.Scaled",infer=TRUE)
```
...
###Is pupae to adult ratio correlated with colony size?
```{r}
summarydata$Pupae.Adult.Ratio <-summarydata$Pupae.Count/summarydata$Adult.Count

RatioSizeModel<-lm(Pupae.Adult.Ratio~Adult.Count, data=subset(summarydata,Colony!=75))
summary(RatioSizeModel)
```
